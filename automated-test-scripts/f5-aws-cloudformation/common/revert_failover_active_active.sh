...
echo "Revoking Ingress rule for 1026 port on internal interface. This is done to make Active-Standby"

response=$(aws ec2 authorize-security-group-ingress --region <REGION> --group-id $SecurityGroup --protocol udp --port 1026 --source-group $SecurityGroup)
