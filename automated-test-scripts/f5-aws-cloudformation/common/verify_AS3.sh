...
1)
  ...
  if [[ "<PUBLIC IP>" == "Yes" ]]; then
    response=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/<SSH KEY>.pem admin@${IP} 'tmsh list mgmt shared iapp installed-packages')
  else
...
if [[ "$number_bigips" = 1 ]]; then
  if echo $response | grep 'f5-appsvcs'; then
    echo "SUCCESS"
  else
    echo "FAILED"
  fi
elif [[ "$number_bigips" = 2 ]]; then
  if echo $response | grep 'f5-appsvcs' && echo $response2 | grep 'f5-appsvcs'; then
    echo "SUCCESS"
  else
    echo "FAILED"
  fi
