...
# grab list of iapps on big-ips
if [[ "<PUBLIC IP>" == "Yes" ]]; then
  response=$(curl -sku <CUSTOM USER>:'<AUTOFILL PASSWORD>' --connect-timeout 10 https://$IP:<MGMT PORT>/mgmt/tm/sys/application/template | jq .items[].name --raw-output)
else
  ...
  response=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/<SSH KEY>.pem ubuntu@${bastion} "curl -sku <CUSTOM USER>:'<AUTOFILL PASSWORD>' --connect-timeout 10 https://$IP:<MGMT PORT>/mgmt/tm/sys/application/template | jq -r .items[].name")
fi
...
2)
  if [[ "<PUBLIC IP>" == "Yes" ]]; then
    response=$(curl -sku <CUSTOM USER>:'<AUTOFILL PASSWORD>' --connect-timeout 10 https://$IP:<MGMT PORT>/mgmt/tm/sys/application/template | jq .items[].name --raw-output)
    response2=$(curl -sku <CUSTOM USER>:'<AUTOFILL PASSWORD>' --connect-timeout 10 https://$IP2:<MGMT PORT>/mgmt/tm/sys/application/template | jq .items[].name --raw-output)
  else
    jq_install=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/<SSH KEY>.pem ubuntu@${bastion} 'sudo apt-get install -y jq')
    echo "JQ Install: $jq_install"
    response=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/<SSH KEY>.pem ubuntu@${bastion} "curl -sku <CUSTOM USER>:'<AUTOFILL PASSWORD>' --connect-timeout 10 https://$IP:<MGMT PORT>/mgmt/tm/sys/application/template | jq -r .items[].name")
    response2=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/<SSH KEY>.pem ubuntu@${bastion} "curl -sku <CUSTOM USER>:'<AUTOFILL PASSWORD>' --connect-timeout 10 https://$IP2:<MGMT PORT>/mgmt/tm/sys/application/template | jq -r .items[].name")
  fi
  echo "Response: $response"
