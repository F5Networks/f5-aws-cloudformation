...
1)
  if [[ "<PUBLIC IP>" == "Yes" ]]; then
    response=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/dewpt_private.pem admin@${IP} 'tmsh list sys ntp')
  else
    response=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/dewpt_private.pem -o ProxyCommand="ssh -o 'StrictHostKeyChecking no' -i /etc/ssl/private/dewpt_private.pem -W %h:%p ubuntu@$bastion" admin@"$IP" 'tmsh list sys ntp')
  fi
  echo "Response: $response"  ;;
2)
  if [[ "<PUBLIC IP>" == "Yes" ]]; then
    response=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/dewpt_private.pem admin@${IP} 'tmsh list sys ntp')
    response2=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/dewpt_private.pem admin@${IP2} 'tmsh list sys ntp')
  else
    response=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/dewpt_private.pem -o ProxyCommand="ssh -o 'StrictHostKeyChecking no' -i /etc/ssl/private/dewpt_private.pem -W %h:%p ubuntu@$bastion" admin@"$IP" 'tmsh list sys ntp')
    response2=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/dewpt_private.pem -o ProxyCommand="ssh -o 'StrictHostKeyChecking no' -i /etc/ssl/private/dewpt_private.pem -W %h:%p ubuntu@$bastion" admin@"$IP2" 'tmsh list sys ntp')
  fi
  echo "Response: $response"
...
if [[ "$number_bigips" = 1 ]]; then
  if echo $response | grep '<NTP SERVER>'; then
    echo "SUCCESS"
  else
    echo "FAILED"
  fi
elif [[ "$number_bigips" = 2 ]]; then
  if echo $response | grep '<NTP SERVER>' && echo $response2 | grep '<NTP SERVER>'; then
    echo "SUCCESS"
  else
    echo "FAILED"
  fi
