...
# disable auto-phonehome
case $number_bigips in
1)
  ssh-keygen -R $IP 2>/dev/null
  if [[ "<PUBLIC IP>" == "Yes" ]]; then
    response=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/<SSH KEY>.pem admin@${IP} 'tmsh list sys software update auto-phonehome')
  else
    response=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/dewpt_private.pem -o ProxyCommand="ssh -o 'StrictHostKeyChecking no' -i /etc/ssl/private/dewpt_private.pem -W %h:%p ubuntu@$bastion" admin@"$IP" 'tmsh list sys software update auto-phonehome')
  fi
  echo "response: $response" ;;
2)
  if [[ "<PUBLIC IP>" == "Yes" ]]; then
    response=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/dewpt_private.pem admin@${IP} 'tmsh list sys software update auto-phonehome')
    response2=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/dewpt_private.pem admin@${IP2} 'tmsh list sys software update auto-phonehome')
  else
    response=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/dewpt_private.pem -o ProxyCommand="ssh -o 'StrictHostKeyChecking no' -i /etc/ssl/private/dewpt_private.pem -W %h:%p ubuntu@$bastion" admin@"$IP" 'tmsh list sys software update auto-phonehome')
    response2=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/dewpt_private.pem -o ProxyCommand="ssh -o 'StrictHostKeyChecking no' -i /etc/ssl/private/dewpt_private.pem -W %h:%p ubuntu@$bastion" admin@"$IP2" 'tmsh list sys software update auto-phonehome')
  fi
  echo "Response: $response"
...
if [[ "$number_bigips" = 1 ]]; then
  ...
  if echo $response | grep $phonehome && echo $response2 | grep $phonehome; then
    echo "SUCCESS"
  else
    echo "FAILED"
  fi
