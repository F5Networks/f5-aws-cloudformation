...
## Introduction
his experimental auto scale solution launches at least one BIG-IP VE Web Application Firewall (WAF) with a BYOL (bring your own license) license, but then uses hourly BIG-IP VEs when auto scale events occur.  The template relies on an existing F5 [BIG-IQ device](https://f5.com/products/big-iq-centralized-management) with a pool of 1-4 BYOL licenses.  But instead of scaling BYOL BIG-IP VE instances as our other auto scale solutions do, as traffic through the BIG-IP VE or the BIG-IP CPU utilization increases, up to 8 hourly BIG-IP VE instances automatically instantiate to seamlessly handle the load (based on values you enter in the template). When traffic or CPU utilization decreases, the hourly BIG-IP VEs are automatically removed.
...
**Important**: You may have to select the AWS region in which you want to deploy after clicking the Launch Stack button.
   ...
   <a href="https://console.aws.amazon.com/cloudformation/home?region=us-east-1#/stacks/new?stackName=F5-Bigiq-Hourly-BIGIP-WAF-Autoscale&templateURL=https://f5-cft.s3.amazonaws.com/f5-bigiq-hourly-autoscale-bigip-waf.template"><img src="https://s3.amazonaws.com/cloudformation-examples/cloudformation-launch-stack.png"/></a>
...
| Maximum Number of Hourly Instances | scalingMaxSize | Yes | Maximum number of Hourly BIG-IP instances (1-8) that can be created in the Auto Scale Group |
