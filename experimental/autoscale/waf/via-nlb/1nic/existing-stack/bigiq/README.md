# Auto scaling the BIG-IP VE Web Application Firewall in AWS: Existing Stack with BIG-IQ Licensing (Frontend via NLB)
...
## Introduction
This experimental template implements auto scaling of BIG-IP Virtual Edition (VE) Web Application Firewall (WAF) systems in Amazon Web Services, using a [BIG-IQ device](https://f5.com/products/big-iq-centralized-management) with a pool of BIG-IP licenses in order to license BIG-IP VEs using BYOL licenses.  
...
![Configuration example](../images/config-diagram-autoscale-waf-bigiq-nlb.png)
