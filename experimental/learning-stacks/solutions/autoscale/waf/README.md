# Auto scaling the BIG-IP VE Web Application Firewall in AWS - Full Stack
...
## Introduction
This solution implements a full stack, auto scaling of BIG-IP Virtual Edition (VE) Web Application Firewall (WAF) systems in Amazon Web Services. The BIG-IP VEs have the <a href="https://f5.com/products/big-ip/local-traffic-manager-ltm">Local Traffic Manager</a> (LTM) and <a href="https://f5.com/products/big-ip/application-security-manager-asm">Application Security Manager</a> (ASM) modules enabled to provide advanced traffic management and web application security functionality.  As traffic increases or decreases, the number of BIG-IP VE WAF instances automatically increases or decreases accordingly. 

This CloudFormation template deploys a full stack, meaning it does not only deploy the BIG-IP auto scale WAF solution, but also the required EC2/VPC resources. In particular, this template creates a VPC, subnets, routing tables, security groups, and two Elastic Load Balancer groups (one to which all BIG-IP VEs are added and one to which the applications servers are added).
...
Note that you **cannot** right-click the link and use "Save as".  You must click **Raw** and save the code with the file name **f5-full-stack-autoscale-bigip.template**.
...
[
                {
                                ...
                                "ParameterValue":"fullF5Autoscale"
                },
                {
                ...
                },
                {
                                ...
                                "ParameterValue":"arn:aws:iam::123456789012:server-certificate/example-website"
                }
]
```
...
| Instance Type | instanceType | Yes | AWS Instance Type (the default is m4.xlarge) |
...
| Maximum Instances | scalingMaxSize | Yes | Maximum number of BIG-IP instances (2-8) that can be created in the Auto Scaling Group (the default is 3) |
...
| Application Pool Member Instance Type | appIstanceType | Yes | EC2 Instance type for the application pool member (the default is t1.micro) |
| SSL Certificate ID | certificateId | Yes | ELB Listener Certificate ID for SSL Offloading |
...
The CloudFormation Template creates and leverages several AWS resources to support Auto Scale, including:
  ...
  - S3 Bucket<br>
    The S3 bucket acts as persistent storage for the cluster database. It contains:
      ...
      The S3 bucket also contains auto-generated credentials for clustering, for example:  ```{"username":"custom-admin","password":"J#\"?}$YDgb8c=L>>P8#FzmS$WB9EYzx3<"} ```
  - IAM Role<br>
