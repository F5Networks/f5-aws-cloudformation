# Deploying the BIG-IP VE in AWS - Clustered 3-NIC across Availability Zones: Existing Stack with BIG-IQ Licensing
...
This solution uses a CloudFormation Template to launch and configure two BIG-IP 3-NIC VEs in a clustered, highly available configuration across Amazon Availability Zones, using a [BIG-IQ device](https://f5.com/products/big-iq-centralized-management) with a pool of BIG-IP licenses in order to license BIG-IP VEs using BYOL licenses.  
...
The BIG-IP VE can detect Availability Zone failure and automatically shift public traffic to the BIG-IP in the Availability Zone that is unaffected. In a 3-NIC implementation, each BIG-IP VE has one interface used for management and data-plane traffic from the Internet, and the second interface connected into the Amazon networks where traffic is processed by the pool members in a traditional two-ARM design. Traffic flows from the BIG-IP VE to the application servers.  
...
The following is a simple configuration diagram for this clustered, 3-NIC deployment. In this diagram, the IP addresses are provided as examples. This solution uses IAM roles for authentication.<br>
![Clustered 3-NIC configuration example](../../images/aws-3nic-cluster-across-azs.png)
